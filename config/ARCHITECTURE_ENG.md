...
| Mode | Steps | `requires_grad` | kWTA | Weight update |
|------|-------|-----------------|------|---------------|
| CHL (Hebbian) | 1–7 | False (default) | Hard | Manual `.data +=` |
| Backprop | 8 | True (set at model level) | Hard | `loss.backward()` + optimizer |
| RSA fitting | future | True (frozen hippocampus) | Soft (`F_kWTA_soft`) | optimizer over embedding only |

**`requires_grad` switching — no constructor flag needed:**
...
```
This avoids scattering mode state between layer constructors and the model.
CHL `update_weights()` always bypasses autograd via `.data +=` and works in either mode.
...
## 6. Theta Oscillation and Phase Structure

### Biological source

The plus/minus phase structure of CHL is grounded in the **hippocampal theta oscillation** (4–12 Hz), generated by the medial septum acting as pacemaker.

Schapiro (2017) §2.b, citing Hasselmo et al. (2002) Neural Comput. [ref 27] and Brankack et al. (1993) [ref 28]:

> "At the trough of the theta cycle, as measured at the hippocampal fissure, EC has a stronger influence on CA1, whereas at the peak, CA3 has a stronger influence on CA1."

| Theta phase | CA1 dominant input | Computational role |
|-------------|-------------------|-------------------|
| Trough | ECin → CA1 | Encoding (external drive) |
| Peak | CA3 → CA1 | Retrieval (internal recurrence) |
