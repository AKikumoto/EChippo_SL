...
# =========================================================================
# TASK EMBEDDING LAYERS (ported from EmbeddingRNN/src/layer.py)
# =========================================================================
# Used to project discrete task-feature indices (rule, stim, resp, conj)
# into rate-coded continuous vectors for use as ECin input.
#
# Why rate-coded ECin is required for K&M task:
#   One-hot per condition → all 16 conditions equidistant → MSP has no
#   signal to learn community-like clustering in CA1.
#   Feature-coded or RSA-initialised embeddings seed ECin with the correct
#   similarity structure (STIM, RULE) so CHL can develop RSRCONJ geometry.
#   See config/ARCHITECTURE_ENG.md §10 for full rationale.
#
# Usage for K&M RuleAction_4rules task:
#   design = pd.read_csv('z_task_design_tables/RuleAction_4rules/Task_Design.txt', sep='\t')
#   units  = design_to_units(design)           # from tasks.py
#   rsa    = units_to_rsa(units)               # from tasks.py
#   emb    = TaskEmbedding(design, units, emb_dim=8, emb_similarity=rsa)
...
def rsa_to_embedding_init(rsa_matrix: np.ndarray, emb_dim: int) -> np.ndarray:
    ...
    Parameters
    ----------
    rsa_matrix : np.ndarray, shape (n, n)
        Symmetric RSA similarity matrix (e.g. from units_to_rsa in tasks.py).
    emb_dim : int
    ...
    np.ndarray, shape (n, emb_dim), dtype float32
        Columns are top eigenvectors scaled by sqrt(eigenvalue).
        Zero-padded if emb_dim > n.
    """
    n = rsa_matrix.shape[0]
    vals, vecs = np.linalg.eigh(rsa_matrix)
    idx        = np.argsort(vals)[::-1]
    vals, vecs = vals[idx], vecs[:, idx]
    vals       = np.maximum(vals, 0.0)          # clip numerical noise

    k = min(emb_dim, n)
    W = vecs[:, :k] * np.sqrt(vals[:k])         # (n, k)
...
class FeatureEmbedding(nn.Module):
    ...
    Each condition index (0..n_cond-1) maps to an emb_dim vector.
    Optionally initialised from an RSA matrix so that conditions sharing
    a feature value start with similar representations.

    Parameters
    ----------
    n_cond : int
        Number of conditions (vocabulary size).
    emb_dim : int
        Embedding dimension.
    rsa_matrix : np.ndarray (n_cond, n_cond) or None
        RSA kernel for weight initialisation. None → default random init.
    freeze_on : bool
...
class TaskEmbedding(nn.Module):
    ...
    FeatureEmbedding. 'CONJ' is handled specially via compositional_on:
    ...
    Output size:
      ...
      shared_space_on=True  → total_emb_dim = emb_dim (features summed
                               after a shared linear projection)

    Parameters
    ----------
    design : pd.DataFrame
        Task_Design.txt loaded as DataFrame; rows=conditions, cols=features.
    units : dict {col: np.ndarray (n_cond, n_values)}
        Output of design_to_units() from tasks.py.
    ...
    emb_similarity : dict {feature: np.ndarray (n_cond, n_cond)} or None
        RSA matrices for initialisation (from units_to_rsa() in tasks.py).
    ...
    def __init__(
        ...
        self._ref_dim = dim_for(base_cols[0]) if base_cols else (emb_dim if isinstance(emb_dim, int) else next(iter(emb_dim.values())))

        self.embeddings = nn.ModuleDict({
            col: FeatureEmbedding(
                n_cond=n_cond,
                emb_dim=dim_for(col),
        ...
        if not compositional_on and 'CONJ' in feature_cols:
            self.conj_emb = FeatureEmbedding(
                n_cond=n_cond,
